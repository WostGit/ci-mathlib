#!/usr/bin/env python3
"""
Route-metadata distillation audit case study.

This experiment is intentionally dependency-free and deterministic.  It models
three audit worlds:

1. Sound post-processing:
   Student = train(Transcript).  The student attack and transcript simulator
   must have identical success.

2. Route-metadata leak:
   Student = train(Transcript, RouteMetadata), but the audit pretends the
   student is only train(Transcript).  This creates a false privacy pass.

3. Conditioning failure:
   Aggregate success can look acceptable while proxy/suspicious subgroups leak
   much more.

The purpose is not to mimic one company's private incident.  It is to make the
security modeling bug concrete: a post-processing theorem requires the exact
training type Transcript -> Student.  If the real type is
Transcript -> AuxData -> Student, the theorem does not apply.
"""
...
# What the internal route metadata reveals.  This is deliberately omitted from
# the audited transcript in the buggy scenario.
ROUTE_TO_CAPABILITY = {
...
def audited_transcript(secret: Dict[str, object]) -> Dict[str, str]:
    """Audited transcript intentionally omits route_id and campaign_id."""
    capability = str(secret["capability"])
    query_family = str(secret["query_family"])

    # Coarse transcript features that reveal some but not all capability info.
...
def auxiliary_metadata(secret: Dict[str, object]) -> Dict[str, str]:
    """Unlogged metadata: route/campaign/account information."""
    return {
...
def train_from_transcript(t: Dict[str, str]) -> str:
    """Deterministic student trained only from audited transcript features."""
    if t["answer_signature"] == "safety":
        return "policy_edge"
    if t["answer_signature"] == "agentic":
        # Coarse transcript cannot distinguish coding/tool/computer reliably.
...
def train_with_route_metadata(t: Dict[str, str], aux: Dict[str, str]) -> str:
    """Buggy training path: uses route metadata outside audited transcript."""
    route = aux["route_id"]
    return ROUTE_TO_CAPABILITY.get(route, train_from_transcript(t))


def train_with_partial_aux_for_proxy(t: Dict[str, str], aux: Dict[str, str]) -> str:
    """Auxiliary leak only for proxy/suspicious subgroups; aggregate can hide it."""
...
def main() -> None:
    ...
    # Example audit budget: transcript simulator plus a small tolerance.
