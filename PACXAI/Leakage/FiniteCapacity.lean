...
def pairObs {Secret A B : Type} (obsA : Observation Secret A) (obsB : Observation Secret B) : Observation Secret (A × B) :=
  fun s => (obsA s, obsB s)

/-- Exact post-processing preserves the pointwise output value. -/
theorem postprocess_output_pointwise {Secret Raw Out : Type}
    (obs : Observation Secret Raw) (f : Raw -> Out) (s : Secret) :
...
theorem pairObs_pointwise {Secret A B : Type}
    (obsA : Observation Secret A) (obsB : Observation Secret B) (s : Secret) :
