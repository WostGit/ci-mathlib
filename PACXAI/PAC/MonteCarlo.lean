...
/-- A finite Monte Carlo audit certificate.  It deliberately uses integer counts rather than floating-point confidence intervals so Lean can check the accounting without external numerical libraries. -/
...
def monteCarloFromAttack {Secret Obs Guess : Type}
    (samples : Population Secret) (criterion : Criterion Secret Guess)
    (obs : Observation Secret Obs) (attack : CandidateAttack Obs Guess)
...
theorem monteCarlo_postprocess_success_eq {Secret Raw Out Guess : Type}
    (samples : Population Secret) (criterion : Criterion Secret Guess)
    (raw : Observation Secret Raw) (f : Raw -> Out) (attack : CandidateAttack Out Guess)
...
def monteCarloFromCandidateClass {Secret Obs Guess : Type}
    (samples : Population Secret) (criterion : Criterion Secret Guess)
    (obs : Observation Secret Obs) (candidates : CandidateClass Obs Guess)
    (allowedSuccesses confidenceNumerator confidenceDenominator : Nat) : MonteCarloCertificate :=
  { samples := samples.length,
...
theorem monteCarlo_candidateClass_postprocess_eq {Secret Raw Out Guess : Type}
    (samples : Population Secret) (criterion : Criterion Secret Guess)
    (raw : Observation Secret Raw) (f : Raw -> Out) (candidates : CandidateClass Out Guess)
    (allowedSuccesses confidenceNumerator confidenceDenominator : Nat) :
