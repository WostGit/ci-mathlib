...
def observedCodeCost {Secret Obs : Type}
    (p : FiniteDist Secret) (obs : Observation Secret Obs) (code : Code Obs) : Nat :=
...
theorem deterministic_dpi_exact {Secret Raw Out : Type}
    (p : FiniteDist Secret) (raw : Observation Secret Raw)
    (f : DetChannel Raw Out) (outCode : Code Out) :
...
def restrictDist {A : Type} (p : FiniteDist A) (condition : A -> Bool) : FiniteDist A where
...
theorem conditional_deterministic_dpi_exact {Secret Raw Out : Type}
    (p : FiniteDist Secret) (condition : Secret -> Bool)
    (raw : Observation Secret Raw) (f : DetChannel Raw Out) (outCode : Code Out) :
...
def infoGainCost {Secret Obs : Type}
    (p : FiniteDist Secret) (obs : Observation Secret Obs)
...
theorem infoGain_postprocess_exact {Secret Raw Out : Type}
    (p : FiniteDist Secret) (raw : Observation Secret Raw)
    (f : DetChannel Raw Out) (baseline observed : Code Out) :
