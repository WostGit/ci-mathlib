...
Interpretation: if a decoder succeeds `successes` times on a population of `total`, then the audit must account for at least `requiredInformation` units of finite leakage/capacity.  The certificate records the arithmetic obligation explicitly. -/
...
This is a log-free, finite recovery certificate rather than Shannon's analytic Fano inequality. -/
...
theorem fanoStyle_student_transcript_success_eq
    ...
    (train : Transcript -> Student) (studentAttack : CandidateAttack Student Guess)
